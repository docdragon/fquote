...
async function populatePrintableArea() {
    ...
    DOM.printableCompanyName.style.color = ''; // Xóa màu động
    ...
    mainTitleEl.style.color = ''; // Xóa màu động
    ...
    if(DOM.printQuoteNotes) {
        DOM.printQuoteNotes.textContent = companySettings.defaultQuoteNotes || "Không có ghi chú.";
    }
    ...
    DOM.printableQuoteArea.querySelectorAll('.signature-title').forEach(title => {
        title.style.color = '';
    });
    ...
    if (printOptions.footer) {
        footerEl.innerHTML = `<hr><p style="text-align:center; font-style:italic; font-size:9pt;">${printOptions.footer}</p>`;

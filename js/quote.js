/**
 * @file quote.js
 * @description Quản lý logic báo giá với listeners thời gian thực và trạng thái.
 */
...
export function listenToCompanySettings(userId) {
    ...
    const unsubscribe = docRef.onSnapshot(doc => {
        if (doc.exists) {
            ...
            // ... (Điền thông tin công ty không đổi)

            // Điền thông tin tùy chỉnh in ấn
            const printOptions = companySettings.printOptions || {};
            DOM.printTitleSettingInput.value = printOptions.title || 'BÁO GIÁ';
            // XÓA BỎ: DOM.printColorSettingInput.value = printOptions.color || '#0056b3';
...
export async function saveCompanySettingsHandler(userId) {
    ...
    // Thu thập dữ liệu tùy chỉnh in ấn
    const printOptions = {
        title: DOM.printTitleSettingInput.value.trim(),
        // XÓA BỎ: color: DOM.printColorSettingInput.value,
    ...
    const settingsData = {
        ...
        printOptions: printOptions // Lưu đối tượng tùy chỉnh
    };
    try {
...
// ... các hàm logic item (add, edit, delete, etc.) không đổi ...

// === SAVED QUOTES ACTIONS ===

/**
 * THÊM MỚI: Nhân bản một báo giá đã có.
 * @param {string} originalQuoteId - ID của báo giá gốc.
...
export async function duplicateQuote(originalQuoteId, userId) {
    ...
    // Tạo một bản sao và thay đổi các thông tin cần thiết
...
// (saveCurrentQuoteToListHandler, deleteQuoteFromList, loadQuoteFromList, etc.)
...
export async function addOrUpdateItemFromForm(userId) {
    ...
    let itemDiscountAmount = (itemDiscountType === 'percent') ? (originalPrice * itemDiscountValue) / 100 : itemDiscountValue;

...
/**
 * Tạo ID báo giá đơn giản theo yêu cầu: Chữ cái đầu tên khách-DDMMYY-XXXX (random).
 * @param {string} customerName - Tên khách hàng.
 * @param {string} quoteDateString - Ngày báo giá (YYYY-MM-DD).
 * @returns {string} ID báo giá.
 */
export function generateSimpleQuoteId(customerName, quoteDateString) {
    let datePart = "DDMMYY";
    if (quoteDateString) {
    ...
    } else { // Fallback if no date string
        const now = new Date();
        const day = String(now.getDate()).padStart(2, '0');
        const month = String(now.getMonth() + 1).padStart(2, '0');
    ...
    const nameInitial = customerName ? customerName.trim().charAt(0).toUpperCase() : 'X';
    const randomSuffix = Math.random().toString(36).substring(2, 6).toUpperCase();
    return `${nameInitial}-${datePart}-${randomSuffix}`;
}
